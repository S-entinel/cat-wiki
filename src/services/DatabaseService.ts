...
export class DatabaseService {
  ...
  private initializeDatabase(): void {
    ...
    this.db.execSync(`
      CREATE TABLE IF NOT EXISTS breeds (
        ...
        tica_code TEXT UNIQUE NOT NULL,
        name TEXT NOT NULL,
  ...
  async getBreedsByOrigin(origin: string): Promise<CatBreed[]> {
    return this.db.getAllSync('SELECT * FROM breeds WHERE origin = ? ORDER BY name', [origin]);
  }

  async getBreedsByActivityLevel(activityLevel: string): Promise<CatBreed[]> {
    return this.db.getAllSync('SELECT * FROM breeds WHERE activity_level = ? ORDER BY name', [activityLevel]);
  }

  async getBreedsByCoatLength(coatLength: string): Promise<CatBreed[]> {
    return this.db.getAllSync('SELECT * FROM breeds WHERE coat_length = ? ORDER BY name', [coatLength]);
  }
  ...
  async updateBreed(id: number, breed: Partial<CatBreed>): Promise<void> {
    const fields = Object.keys(breed).filter(key => 
      key !== 'id' && 
      key !== 'created_at' && 
      key !== 'updated_at' && 
      breed[key as keyof CatBreed] !== undefined
    );
    ...
    const values = fields.map(field => breed[field as keyof CatBreed]).filter(value => value !== undefined);
  ...
  private async populateInitialData(): Promise<void> {
    const { ticaBreedsData } = await import('../data/ticaBreedsData');
    
    for (const breed of ticaBreedsData) {
      try {
    ...
    console.log(`🎉 Database seeded with ${ticaBreedsData.length} TICA breeds!`);
  }
}